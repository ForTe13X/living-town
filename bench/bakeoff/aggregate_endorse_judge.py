#!/usr/bin/env python
# Aggregate the faction-moment blind judge (endorse vs abstain). Maps each blind A/B back via the
# held-out mapping, combines mirror pairs, cluster-bootstraps p_eff(endorse) by seed, and breaks it
...
SCORE={"endorse":1.0,"tie":0.5,"abstain":0.0}
...
print("="*64)
print("FACTION-MOMENT BLIND JUDGE — endorse (coordinated badmouth) vs abstain")
print("="*64)
print("  cases=%d seeds=%s mirror-consistency=%.0f%%"%(n,seeds,100*consistent/n))
print("  raw: endorse=%d abstain=%d tie=%d"%(raw["endorse"],raw["abstain"],raw["tie"]))
print("  A=A controls: %d/%d judged 平"%(ctl_tie,len(ctl)))
print("  p_eff(ENDORSE) = %.3f  95%% CI [%.3f, %.3f]"%(peff,lo,hi))
verdict=("favors ENDORSE (logic right)" if lo>0.5 else
         "favors ABSTAIN (logic endorses out-of-character; DSL should default to abstain)" if hi<0.5 else
         "WASH")
print("  DIRECTION: %s"%verdict)
print("  ── per-persona p_eff(endorse) (who, if anyone, is in-character endorsing) ──")
g=defaultdict(list)
...
print("="*64)
cw=[v for v in verdicts if winner(v)=="endorse" and not v["tid"].endswith("#ctl")][:5]
print("sample reasons — judge chose ENDORSE:")
