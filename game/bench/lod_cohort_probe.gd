...
## 观察无关 aggregate LOD 下：每 tick 读 _near_set/_pact_ids/... 尺寸 → 报 cohort 大小(成本)、
## 各 salience 贡献者规模、以及【每 agent 是否 span 内必满帧一次】(无永久冻结)。纯读 sim 公开字段，不改 sim。
## 并与全量 sim(lod_aggregate=false)对比 cand_calls，如实报 constant-factor 而非渐进。
## 用法：--script res://bench/lod_cohort_probe.gd -- [N] [days] [seed]
...
func _init():
	...
	var total: int = days * int(full.TICKS_PER_DAY)
	for t in range(total): full.tick()
	var full_cand: int = int(full.cand_calls)
	full.free()

	# ── 观察无关 aggregate LOD ──
	var S := _mk(N, true); S.start_new(seed)
	var span: int = int(S.lod_rotate_span)
	var sum_full := 0.0; var sum_pact := 0.0; var sum_commit := 0.0; var sum_conflict := 0.0
	var last_full := {}          # id -> 最近满帧 tick
	var max_gap := 0             # 全程任一 agent 距上次满帧的最大间隔（应 < span → 无永久冻结）
	var never_full := {}         # 从未满帧过的 id（span 结束后应为空）
	for ag in S.agents: never_full[ag["id"]] = true
	for t in range(total):
		S.tick()
		sum_full += float(S._near_set.size())
		sum_pact += float(S._pact_ids.size())
		sum_commit += float(S._commit_ids.size())
		sum_conflict += float(S._conflict_ids.size())
		for id in S._near_set:
			never_full.erase(id)
			if last_full.has(id):
				max_gap = maxi(max_gap, t - int(last_full[id]))
			last_full[id] = t
	# 收尾：对每个曾满帧的 agent，补算「最后一次满帧到终点」的尾间隔
	for id in last_full:
	...
	print("  salience 贡献者端点数: pact=%.1f  commit=%.1f  conflict=%.1f" % [sum_pact/ticks, sum_commit/ticks, sum_conflict/ticks])
	...
	print("  全量 sim: %d   观察无关 LOD: %d   → 省 %.0f%%（constant-factor，非渐进；数千 NPC 需正交空间分桶，超本工作范围）" % [
