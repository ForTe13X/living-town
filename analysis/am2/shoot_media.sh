#!/usr/bin/env bash
# shoot_media.sh — AM2 一次性媒体渲染（docker gamecraft-runner，Xvfb+opengl3 软渲，与 visual_gate 同镜像）。
# 用法（host，Git Bash）：  bash analysis/am2/shoot_media.sh <out_dir_hostwin> <tag> [game_dir_hostwin]
#   在 <out_dir> 落 shop_1f_<tag>.png / work_1f_<tag>.png（真引擎 --shot，红线 R2：非生成图）。
#   game_dir 可指向别的一棵 game/（拍 before：给未改动树的 game/）。默认本仓库 game/。
...
set -e
Xvfb :97 -screen 0 1280x768x24 -nolisten tcp >/tmp/xvfb.log 2>&1 & sleep 1.6
export DISPLAY=:97 LIBGL_ALWAYS_SOFTWARE=1 LP_NUM_THREADS=1 GODOT_SILENCE_ROOT_WARNING=1
for sid in shop work; do
