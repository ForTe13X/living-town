...
**还不到位的**（详见 [docs/05](docs/05-路线图与里程碑.md)、[docs/43](docs/43-wave-c-plan.md)）：游戏是**全哑的**（无任何音频）；手机上只能看不能玩——7 个玩家动词不但全是键盘绑定，还锁在一个 `--player` 启动旗标后面，而出货目标是 Android APK；开局取景只看得见约 14% 的地图，全镇视角里约 58% 的画面是引擎默认清屏色；居民没有位移插值（每秒瞬移 12.5 次）；四季与天气在仿真里逐日生效，但渲染层逐像素相同；没有目标、没有引导、没有单局形状。
...
- **⚠️ 一条出货阻断项，也是 CI 结构性看不见的**：`backend=slm` 下硬不变量 **#01「无饿穿」在 8/8 个 seed 上被违反**（同配置 logic 是 0/8）。因为 CI 钉 `Sim.backend=null`，**模型路径上的硬不变量从未被任何门检查过**。已排除犹豫/串行化（`mock` 在同样的延迟与空转率下不饿死任何人）——是**选择本身**让 agent 不去吃饭。红线#2「无模型也能玩」安全，但**"开着模型时红线#1 依然成立"这件事，此前只是被默认继承，并不为真**。修复与加门列为下一棒。
