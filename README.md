...
下列**世界与社会系统**都已出货，且各自有 CI 门守着——点名不变量编号的由 [`game/bench/Invariants.gd`](game/bench/Invariants.gd) 守，其余由 [`tools/ci.sh`](tools/ci.sh) 的数据 lint / 地图审计 / 美术门 / 台词门 / 9 个集成场景守。末三条（外壳、后端、推理延迟）是实测记录，不是不变量。
...
- **分工有自己的地方**：木匠/杂役/渔夫/教书先生各有工位，商贩把镇级库存转成个人可买的货（实测 **141-165 次 `transfer`/seed/60 天**，约占全镇饭口 **15%**），环卫工按整洁度自己成环（广场干净就去玩，脏到约 **45%** 以下去扫）。
...
**还不到位的**——**这一节是按最近一波的实测重写的，不是沿用上一版**（详见 [docs/05](docs/05-路线图与里程碑.md)、[docs/49](docs/49-wave-g-plan.md)）：

- **没有目标、没有引导、没有单局形状**——第 3 分钟没有任何东西把玩家叫回来。**这仍然是最大的一条**，
  而且三路评审（外部对抗模型 + 两个独立只读 agent，指令均为"REFUTE"）的判决是同一句：
  **"它看起来明显更好了，而且仍然没有人会玩它。"**（[docs/43](docs/43-wave-c-plan.md)）
- **镇上的两个池塘是两块纯色贴纸。** 真机帧上北池取样 110×70 px ⇒ **7700 px 全是同一种颜色**；
  `game/assets/art/terrain/water.png` 是 **16×16、1 种颜色**，而同目录的 `grass_b` / `grass_flowers` **各有 4 种**；
  水**只有这一张贴图**，没有 b 版、没有岸边过渡瓦 ⇒ 两片 8×5 的水域渲染成**硬 90° 直角的纯色矩形**，
  直接压在草地上、没有任何岸线。**讽刺的是**：`WorldView` 的水层有一条更好看的回退路（纹理缺失时画深水色并点静态涟漪），
  **它永远不会跑，因为贴图存在**——一份更差的素材让一段更好的代码变成死代码。
  这不是本波的回归，`water.png` 从 6 月 27 日起没改过；**只是从来没有人拿新鲜眼睛看过一帧真机画面。**（[docs/49 §六](docs/49-wave-g-plan.md)）
- **新的美术门只盖住十分之一。** 它守的是 `game/assets/art/pro/` 那 **10 张**角色表；
  `game/assets/art/` 下另有 **31 张**出货 png（emote 10 / decor 8 / terrain 5 / obj 5 / building 3）
  仍然**一道门都没有**。（[docs/49 §七](docs/49-wave-g-plan.md)）
- **上一波修掉的美术缺陷，人眼在 2× 下看不出来。** 补完 144 px 描边之后，引擎内 **983040 像素里只变了 40 个（0.0041%）**，
...
- 手机上仍有 **24.6% 的黑边**（`aspect=keep`；换 `expand` 在真机上黑屏，已回滚）——
  本波在 Wave G 的真机帧上重新量过：2688 px 宽的屏幕，**内容区 x=331..2356，左右纯黑 662 px = 24.6%**。（[docs/46 §二·九](docs/46-wave-d-plan.md)）
...
> 纪事面板在讲 2/11 段故事；澡堂的浴池/柴垛、工坊的两张工作台都是真精灵，**没有一个占位框、草地上没有一行数据键**。
> 这张图**同时是上面「还不到位的」里两条的证据**：左右两条纯黑边就是那 24.6%，
> 而地图上下那两块**边缘完全笔直的青色矩形**就是所谓的池塘。
...
**14 道关**（步骤号即 `tools/ci.sh` 里的编号）：`0` 版权红线（整棵已跟踪树里不得有权重/二进制）、`1` 数据 lint、`1b` 地图审计、`2` markdown 链接 lint、`2b` **美术门**（出货 `pro/` == 当场重建）、`3` Godot 解析冒烟、`4` **S0 不变量门**（40 条 × 12 seed × 60 天 + determinism 三跑 + **已提交金标**跨进程锚 + **逐 tick 前缀链** + 套件级活性）、`4b` LOD 观察无关门、`4c` **DetGate 场景确定性门**（default/faction/betray/freerider 四轨）、`4d` BackendGate 外部后端门、`4e` ModelPathGate 出货 prompt 编码门、`4f` **台词覆盖门**、`5` 9 个集成场景、`6` 视觉门（昼夜量具 + 界外重画 + 空间往返；**无渲染环境时自动 SKIP，不假红**）。任一红即退出 1。
...
**全部 48 篇编号文档的分主题索引见 [docs/README.md](docs/README.md)。** 文档主要为中文。
