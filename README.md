...
手机上仍有 **24.6% 的黑边**（`aspect=keep`；换 `expand` 在真机上黑屏，已回滚——实测内容区 x=331..2356）；
去道具之后**十张脸共用同一个轮廓**（此前约四类），这是 alpha 遮罩的必然代价，替代轮廓还没画（[docs/44](docs/44-art-direction.md)）；
音频**没有人真的戴耳机听过**——所有证据都是 RMS 与窗口计数；
触屏动作条**没有在真机上按过**。
