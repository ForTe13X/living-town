...
> 桌面实录，`logic` 后端，**一个完整的昼夜循环**（1280×768 原尺寸，[完整片段 1:10 带声音](docs/media/town_wavec_demo.mp4)）。
...
![白天的小镇](docs/media/wavec_town_noon.png)

> 同一构建的静帧。地图不再是灰色虚空里的一块矩形——边缘经 3 格亮度斜坡、矮石墙与排水沟没入林中，
