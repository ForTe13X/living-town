...
- **端到端眼验（CPU 版 overlay，跑到第 19 sim-日）**：`后端 slm·并发 1 · 发起 51·成功 34·超时 1·无效 15`——**落地率 ~67%（34/51）**、超时仅 1、**熔断未跳·SLM 持续产出**（对比 GPU/旧版=成功 1 后熔断冻死）。即**手机上 NPC 决策 2/3 真由端侧 SLM 驱动**（其余 15 无效=1.5B 偶回 prose 非纯编号，parse_decision 兜底、优雅退 logic）。〔⚠ **这句话是错的——分母错了。见本文末「更正 ③」与 [docs/35](35-slm-decision-share-and-lod-soft-gate.md)。67% 是 landed/FIRED，不是 landed/全镇决策；真实占比约 2%。**〕**诚实权衡**：CPU 推理活跃时 FPS 88→34（推理与游戏主循环抢 CPU 核；决策已按 host 档节流，34 FPS 仍可玩）——若要更高 FPS 可换更小模型/限核/降节流频率。
...
## ⚠️ 更正 ③（2026-07-25）：「2/3 的 NPC 决策由端侧 SLM 驱动」——**分母用错了，真实占比约 2%**

上面那句 `即手机上 NPC 决策 2/3 真由端侧 SLM 驱动` 是从 `发起51·成功34` 这个 **66.7% = landed/FIRED**
推出来的。这一步推理无效，且错得系统性——不是小数点问题，是**差了一个数量级**。

**错在哪。** landed/fired 回答的是「**我们选择发出去的**请求里，有多少按时回来了」。
而「全镇决策有几成是模型做的」，分母必须是**全镇实际落地的决策总数**。两者差的正是那一大片
「因为串行 worker 正忙、我们干脆没发」的决策——它既不进分子，也不进分母，于是完全隐形。
`AIBackend.decide` 在 worker 忙时直接 `return Sim._logic_decide`（AIBackend.gd 的 `_slm_busy` 门），
这些**是**决策、而且是引擎做的，却从没被算进任何比率。

**串行 worker 的墙钟天花板（与 N 无关）。** `_slm_submit` 一次只放一发；
1 sim-日 = `TICKS_PER_DAY(240) × tick_interval(0.08s)` = **19.2 s 实时**
（`Sim._process` 用累加器，掉帧只会一帧内多补几个 tick，不会把 sim-日拉长）。
故单发解码 D 秒 ⇒ 全镇每 sim-日最多 `19.2/D` 次模型决策，**和镇上有多少人完全无关**：

| 解码 D | 全镇上限（次/sim-日） |
|---|---|
| 3 s（8Elite CPU 暖发下限） | 6.4 |
| 8 s（暖发上限） | 2.4 |

而分母随 N 线性涨（实测见下）。所以 N 越大真实占比越低——**但 landed/fired 完全看不出这件事**。

**用本文自己的真机数据重算。** 上面那次 CPU 眼验是 `发起51·成功34`、**跑到第 19 sim-日**：
- 模型决策 = 34 / 19 ≈ **1.79 次/sim-日**（发起 51/19 ≈ 2.68 次/sim-日，正落在上表 [2.4, 6.4] 区间内 → 天花板与实测互相印证）
- 全镇决策总数（桌面确定性实测，backend=logic，出货默认 cast=12）= **87.2 次/sim-日**
- ⇒ **真实决策占比 ≈ 1.79 / 87.2 ≈ 2%**（若改用 SLM 在跑时的分母 76.3，则 2.3%；量级不变）

**即：端侧 SLM 大约做了全镇 2% 的决策，不是 2/3。** 剩下 98% 一直是确定性引擎地板在做——
这恰恰是红线 #2（无模型也能玩）在起作用，本身不是坏消息；坏消息只是那句话把它说反了。

**这不否定什么。** 「67% 的**已发出**调用落在截止线内」仍然成立，而且仍是有用的指标——
它量的是延迟工程（池化/CPU 默认/deadline 15s）做得好不好，那部分工作是真的。
被推翻的只有从它跨到「决策占比」的那一步。

**方法论。** 这正是本项目一再自查、也一再犯的同一个错：**挑了方便的分母**
（见 [[feedback-adversarial-external-review]]、docs/31 里 #15 的时间泄漏）。
两次都不是算错，是**没问「这个比率的分母到底是什么」**。
完整推导、复现命令、以及 N=12/N=60 的桌面实测，见 [docs/35](35-slm-decision-share-and-lod-soft-gate.md)。

> 注：本文最后那次「合并版装机眼验」`发起22·成功13·超时0` **没有记录跑了几个 sim-日**，
> 因此**算不出**它的决策占比——这里不补一个猜的数字。要补就重跑一次并记下 sim-日数。
