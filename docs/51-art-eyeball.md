...
> ⚠️ **2026-07-30 更正（H2 抓到，主 session 复核）：上面这段整个是错的，而且错得很典型。**
> 原文写的是：「13+11+4+1 = 29 > 26 …… **表格加不齐，是因为现实加不齐**，不是我数错了」。
> **就是数错了。** OK 那一档实际是 **10** 张（decor 6 + obj 3 + emote 1），不是 13：
> **10 + 11 + 4 + 1 = 26**，与出货张数**正好对齐**，根本没有需要解释的差额。
>
> 值得记住的不是这个笔误，是**它引出来的那段辩护**：一个算错的数造出一个不存在的矛盾，
> 而我没有回头数一遍，反而**为这个矛盾发明了一套机制**（「四个桶不足以同时表达死与坏」），
> 还特意写下「不是我数错了」。**加不齐的时候，先重新数，再谈现实。**
>
> 这个错还传染了：docs/50 §二 抄走了 13 并加了个「等」字——
> 若照抄执行，H2 会去给那 9 张我自己判为「读不出」的表情上门。
