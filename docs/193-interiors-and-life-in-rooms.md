...
1. **Sim 的路径是 4 连通的**（`_step_toward` 先 x 后 y；A* 的"斜线"是 E,S,E,S 的阶梯）。旧版逐步取向 ⇒ 斜着走的人每一格在"朝东/朝南"之间翻身。
   现在朝向 = **朝着要去的地方**：有目标（去用某件家具 / 找某个人说话）且在同一平面 ⇒ 取【目标 − 当前格】的 8 向（走阶梯时恒为东南，不翻）；
   没有目标才退回上一步的方向（`WorldView._face_step` / `_goal_cell`）。换 Space/Floor、瞬移（≥3 格）不算一步、不转身。
   ★第一版用"最近 4 步合位移"——那份记忆跨过读档/回放留在 View 里，回放到同一 tick 的朝向与现场不同；现在的规则**无记忆**（只读本 tick 的 Sim 状态）。
...
| 设施 | 能做的事 |
|---|---|
| 面包房、可丽饼店 | 小圆桌「吃点心」（不在 `production.consume` 里 ⇒ 不从镇库口粮出账；第一版用「吃饭」，N=16 口粮满足率掉到 0.31）· 墙上的画「赏画」 |
| 礼拜堂 | 长椅「静坐」、祭台「祈祷」 |
| 室内市场 | 摊位「逛集」 |
| 海滨大酒店 | 钢琴「弹琴」、沙发「歇着」、墙上的画「赏画」 |

### 出门去公共场所（Sim `_journey_candidates` A2）

设施建好之后先量了一次（seed 3，60 天，逐对象"开始用"次数）：N=12 与 N=24 下五座设施、澡堂与图书馆室内的家具**一次都没被用过**。
原因在旧设计：镇上居民只会拐进咖啡馆（常客 + fun 低的 A 块行程），别的楼里的家具只有本来就在楼里的人会用。

⇒ 加一条与 A 同形的行程候选：`spaces.json` 里 `public_venue=true` 的 Space（五座设施 + 澡堂 + 图书馆），
镇上居民在**不紧急**（min_need ≥ SURVIVAL_GATE）且 hunger 或 hygiene 已掉到 55 以下（`VENUE_URGE=45`）时考虑去那里（吃点心、泡澡），
与镇上对象同一套打分（urgency×amount/60 − 路程×penalty）公平竞争，不加"出门走走"的分（`VENUE_VISIT_BONUS=0`）。

第一版也为 fun 出门，同一探针（N=12，seed 3，60 天）：澡堂浴缸「泡澡」255 · 面包房「吃点心」271 · 墙上的画「赏画」356 ·
可丽饼店「吃点心」33 · 酒店钢琴「弹琴」19 —— 但 fun 类行程抢走了工位广告（工位也是 fun），上工下滑、镇库断链，
S0 N=12 的 #40 从 12/12 掉到 **2/12**。⇒ 收窄为只为 hunger/hygiene 出门；画/钢琴由已经在楼里的人顺手用。
礼拜堂、市场、酒店因此主要是"看得见、进得去"，而不是日常去处——留作下一刀（需要连着工位吸引力一起标定）。

### 供给再平衡（用户选择"修根因"而不是带红开 PR）

多了去处之后，#40（产出闭环 / 供给充足，软门 ≥11/12）贴着边：出货树本身在 N=16 就恰好 11/12。改动后逐变体实测（seeds 1-12 × 60 天，det 1）：

| 变体 | N=12 | N=16 |
|---|---|---|
| 出门也为 fun，+4 分 | 2/12 | — |
| 只为 hunger/hygiene，URGE 30，+4 分 | 8/12 | 12/12 |
| URGE 45，+4 | 10/12 | ≥11/12 |
| URGE 30，+0 | 9/12 | 9/12 |
| URGE 45，+0 | ≥11/12 | 10/12 |
| 面点师批量 90→100（撤回） | — | 9/12 |
| **URGE 45，+0，工位广告 +4（采用）** | **≥11/12** | **≥11/12** |
| URGE 45，+0，工位广告 +8 | ≥11/12 | ≥11/12 |

逐动作对照（N=16，seed 3）找到的根因：新去处分走了一部分注意力（喝咖啡 144→174、泡澡 0→90），**本职上工被挤掉**
（劈柴 16→0、修屋 49→32、做点心 48→34）。修法取最小的那一档：`production.json` 九条带 job 的工位广告 amount 各 +4
（刨木 34→38 … 做点心 48→52），让"上工"在多了去处的镇上仍然抢得过闲事。只调面点师一个人的批量反而更差（非单调），已撤回。
...
## 回放像素门（SpaceShot clean-player）的两处 View 修正

- 姿势只读 Sim 状态（phase=="use"），不读插值进度 `_moving`。
- 时间轴跳转/回放（`Sim.replaying`）期间重放出的社交事件不生成气泡/表情：启动 warmup 的跳转发生在 View 接上信号之前，
  现场从来没有那几秒的气泡；读档后的 `goto_tick` 却会把同一段重放出来、多出一批气泡 ⇒ 同一 tick 两种画面。
  这是潜伏的不对称，出货树的 sim 轨迹恰好没在那个窗口里有人说话；本分支轨迹不同就撞上了（本机实测 master pixels_exact=true、本分支修前 false）。
