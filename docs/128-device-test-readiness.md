...
| 纪事查看 | `J` | 可点的纪事摘要 | ⬜ 待测 |
| 存/读档 | `F5`/`F8` | 设置面板存/读档钮 | ✅**当前 HEAD 实测(存)**：点「存档 F5」→写出 `quicksave.dat` 2.67 MB（124 天全态）；读档钮触摸可达（round-trip 未跑） |
...
6. ⬜ **仍未测**：读档 round-trip（存了没回读验状态）、选居民卡、**多楼层室内的触摸**（AG3 纵切在真机上）、HUD 2.21:1 逐项排版、音频主观质量/路由/并发 focus。留给下次。
