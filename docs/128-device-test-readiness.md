...
8. ✅ **第五轮（2026-08-07）·当前 HEAD（`a063457`，含 AG3 cafe 内容）真机验证**（Codex Phase 1.4）：SDK-free 重建 APK（sha256 `29d1e175…`，预建模板+自建 debug key）装 NX789J → app 跑、town 渲染、Tab选+L跟随 → **镜头切进 cafe 室内**：顶栏「咖啡馆/后厨/茶座」分区 + 咖啡机/柜台（=AM1 cafe 1F 内容真机渲染），且**整镇人在 cafe 社交**（苏琴/老邓/沈书/铁牛/阿丽 一堆人吃饭+聊天=事件舞台）。⇒ **新 cafe 内容真机渲染+人流真实已验**。
9. ⬜ **仍未测**：楼梯上 2F 那一跳单独走（连接不稳、没多点；室内渲染路径本轮已由 cafe 1F 证）；HUD 2.21:1 逐项排版；音频主观质量/路由/并发 focus。留给下次。
