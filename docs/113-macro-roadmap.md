...
- **wiki（⬇ 延后）**：**镇民百科**（NPC 职业/关系/信念/大事记，从 `event_log`+`beliefs` 生成的只读投影，不进金标）。AD1 已落地过基座（town_wiki/v1）。⚠️ Codex §三.7：wiki 冲突少但**不是当前最高产品价值**——选它是在优化"容易绿"而非产品目标。**延到事件结果 schema 与 state projection 稳定之后**再做深度增强。⚠️但**架构轨全部 §0.8-gated 待用户拍板期间**，非门控功能轨里 visual 已成熟（AF2+AI1）、map/interior 是 gated 的最高价值轨 ⇒ **AH 阶段选了个非重复的横切增强填坑：AJ1（编号 130）在跑**——镇级总览（关系图谱 SVG + 大事年表 + 派系/冲突概览，纯渲染器从已有 town_wiki/v1 JSON 建，零仿真改动）。这是"非门控可做的里挑非重复的"，不是"最高价值"（最高价值仍是 gated 的 §一/§二/map-interior）。
