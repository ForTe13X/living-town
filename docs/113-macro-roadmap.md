...
- ✅**F4（我的记录在案失败模式，已收）**：多处"已收口/CI PASS"证据超出留档——docs/113 内 AE2 自相矛盾（已对齐单一判决）、"1286s/1596s/1377s" 无归档的精确秒数（已删/降级）、docs/134 "INTSHELL/FURNROLE 全过"但 log 只有采集行（已降级为结构论证 + 待整轮 CI 归档回填）、docs/133 引不存在的 `golden_*.log`（已改指真存在的 `digests_*.txt`）。**统一纪律：判决行以 2026-08-07 F1+AM4 整轮 CI 归档为准回填。**
...
- 🎯**F2 = 下一根旗舰棒（top next action）**：**室内有家具但没人**——`WorldView` 只在 `space=='town'` 画 agent 层，进 cafe 看到空房，可阿丽真住 cafe/2f。这是"活的像素小镇"半截货。修：interior 画完家具后加住户层（`ag.space==active_space && ag.floor==active_floor`，floor-local pos 已在 agent 上），单文件、零金标（Sim 不读 draw 侧）。**AM4（WorldView.gd 装饰 slot）在跑、同文件冲突 ⇒ F2 排在 AM4 落地之后**。
- 🚦**F3 = 纪律（已内化）**：**冻结新造 bespoke per-slice 门 / oracle 扩张**，直到出现现役门抓不到的、玩家可达的产品 bug 才建门。切片验收=一次桌面眼验 + 现役视觉门。下一波投玩家可见的室内生命（F2），不是第 4 间空房、也不是新门。
