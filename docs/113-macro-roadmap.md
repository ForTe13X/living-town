...
2. **exact-head CI**：⚠️ **GHA 超时 15 分钟，而本地全量 CI ~17-26 分钟**⇒ PR #5 每次都在 `story_test` 处被平台取消（Codex 实测两次 run 都 `cancelled@15m`，唯一红因是预算不是断言）。
   ...
   - **正式**：✅ **2026-09-15 落地，见 [docs/205](205-ci-parallel-lanes.md)** —— 拆成七条并行 lane job（`lint`/`s0`/`pool`/`gates`/`backend`/`scenes`/`story`），`ci` 改成汇总 job 以保住分支保护里的 required context；关键路径 42m26s → 约 11 分钟，计费只多 5%。与下面这段原始设想的**两点出入（照实记）**：① 没有做 phase manifest —— lane 表就写在 `tools/ci.sh` 里，workflow 只引用它，由新的 1c 对账门（`tools/assert_ci_lanes.py`）逐字核对，于是本来就不存在"两份门列表"；② 划分不是按 static/provenance·core-sim·… 的语义分类，而是按**实测代价**切在天然缝上（最贵的 `story_test` 独占一条 lane，它 633s 就是关键路径的下界）。原始设想如下：
   - 拆成**全部 required 的并行 jobs**（static/provenance · core-sim · deterministic-backends · scenes · rendered/device），每 job **独立 clean checkout 不共享可变工作目录**，由**同一 phase manifest** 驱动本地全量与 GHA matrix（避免维护两份门列表）。扩展 seed 网格才留给 nightly。
