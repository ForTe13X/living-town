...
本树上有编号文档 00-25、28-47（26 与 27 只存在于未并入的分支，见下）+ 若干附录。
...
| [43 Wave C 计划](43-wave-c-plan.md) | **当前这一波的权威计划**：从「研究仓库」转向「像个游戏」；路线图差量、共同规则 R1-R8、逐棒 brief、逐棒回执与**被执行者证伪的验收标准** |
| [46 Wave D 计划](46-wave-d-plan.md) | **当前这一波的权威计划**：从「看得清」到「有人想玩」。三路评审的判决是「它看起来明显更好了，而且仍然没有人会玩它」⇒ 新增 R9（不许用画面指标替代玩法证据）与 R10（每一波必须有一次「有人把整帧看一遍」）；按杠杆排序的八条剩余缺口与逐棒 brief |
| [47 Wave E 计划](47-wave-e-plan.md) | **当前这一波的权威计划**：让劳动真的产出东西（实测现状是分工只有【行为】没有【产出】——把全镇工作行为全删掉，除了钱包数字世界逐格相同），并把事件链讲成故事。新增 R12：会移动 digest 的棒，验收条件从「金标不动」换成「金标动得能解释」 |
| [44 美术方向与目标调色板](44-art-direction.md) | 角色收向「现代日常小镇居民」（CC0 允许改绘 ⇒ 代价比"重做整套"低一档）；40 色目标 `.gpl`；**实测代码里有 134 个硬编码色值 ⇒ A-art-0 是一次收敛重构，不是写一个文件** |
