...
本树上有编号文档 **00-25、28-51、53-67**（**26 与 27 只存在于未并入的分支**，见下；**52 号从未被占用**，见下）+ 若干附录。
...
> **索引本身也会漂。** 本文上一版停在 48 号，而树上当时已经到 66——中间 20 篇（含 36/37 两篇更早的）
> 一篇都不在索引里。这一版把它们补齐了。**"索引落后"不是排版问题**：`_gpt55-review-brief.md` 之外，
> 索引是外部读者唯一的入口，落后 20 篇等于把最近三周的工作全部藏起来。
...
| [13 实验札记](13-实验札记-experiment-journey.md) | 1700+ 行的过程日志：发现、奇技、坑、**被推翻的结论**。仓库里最能看出工程判断的一篇 |
...
| [67 Wave R](67-wave-r-plan.md) | 还 L2 的两笔账；给 `#40` 补上限臂；**把这份索引追回来**（就是本文这一版） |
| [70 Wave R · R3 回执](70-wave-r-r3-docs-receipt.md) | 索引 / 阶段报告 / 札记这一棒的回执，含**它没能做到的那一条**（`lint_links` 在本棒树上仍是红的，原因不在本棒） |
