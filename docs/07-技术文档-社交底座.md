...
| [Art.gd](../game/scripts/Art.gd) | 调色板 + 中文字体加载（`assets/fonts/smiley-sans.ttf` 运行时喂字节） |
...
- **中文渲染**：项目自带 `assets/fonts/smiley-sans.ttf`（得意黑 Smiley Sans, OFL-1.1），`Art.font()` 运行时喂字节加载——绕过「未导入项目无 .ttf 资源 / 容器无 CJK 字体」的 headless 坑。
