...
| 生活模式·说法（阿丽走近沈书，菜单标「怎么开口 [模型]」，4 条合法行、贴人设） | **5.4 s**（~90 token 生成） |
| 生活模式·对话（选第 1 条开口 → 沈书用模型回一句，接住了"咖啡"话头） | **1.3 s** |
...
- 游戏内说法/对话真机各验了**一发**（5.4 s / 1.3 s，见 §5），不是分布；说法 5.4 s 期间菜单先显示规则地板、回包后替换，玩家不空等。
