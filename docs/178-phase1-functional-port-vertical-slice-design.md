...
- **负控 / 验收**：`tools/test-run-godot-supervised.ps1` 真跑 P1-d focused scene（exit0）、故意让主循环超时（exit124）并在 owner 运行时启动第二实例（exit78）。最终三条 receipt 分别为 `176d96d…` / `f51c256…` / `697c13c…`，均钉在 source `0cfc495`、game tree `1d16ae5`，native-crash=false、cleanup=true；PowerShell parser 与 `git diff --check` 通过。完整接口、来源与恢复证据见 `analysis/p1f/supervised-godot-runner.md`。
