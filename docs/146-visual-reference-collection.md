...
| 2 | `ref_terrain_v1_stardew.png` | 7 类地砖参考表，每类 7 变体（center/alt center/edge/corner/transition/mixed·broken/patchy·worn）：**GRASS / DIRT PATH / STONE PAVEMENT·PLAZA / FARMLAND SOIL / RIVER·WATER EDGE / SAND / WOODEN BOARDWALK** | GPT img-gen（chrome session, 2026-08-07；2026-08-08 重取入库） | terrain 重铺风格锚（草地/土路/石铺·广场直接衔接 AP1/AP2 已做的石街·广场；农田/水岸/木栈道给未来农业·港口） |

> ✅**2026-08-08 terrain 已重取入库**：首次提交因 ChatGPT `download` 按钮开的是全屏预览而非下载、按时间戳误抓了另一项目(Jianghu)角色图（AT1 实读抓到），terrain 行一度撤下。现用**页内 JS `fetch(img.src)→blob→a.download`**（memory [[reference-chrome-imggen-retrieval]]）从 chrome ChatGPT 会话「像素美术参考图」取回真图（1448×1086、2.64MB、image/png、字节数与页内 blob 精确一致），落盘 **Read 眼验确为 7 类地砖表**（非误图）后入库。buildings 同法早前已修正。两张参考现均在库、已核。
