...
step "0a. cafe authored manifest gate (stable IDs -> canonical manifest + negative tooth)"
CAFE_IDS="evidence/cafe/cafe-authored-ids.v1.json"
CAFE_MANIFEST="evidence/cafe/cafe-authored-manifest.v1.json"
if [ -f "$CAFE_IDS" ] || [ -f "$CAFE_MANIFEST" ]; then
  if [ ! -f "$CAFE_IDS" ] || [ ! -f "$CAFE_MANIFEST" ]; then
    bad "cafe authored bundle incomplete (stable IDs and generated manifest must travel together)"
  else
    CAFE_COMPILED="$LT_LOG/cafe-authored-manifest.compiled.json"
    CAFE_POSITIVE_LOG="$LT_LOG/cafe-authored-manifest.positive.log"
    CAFE_NEGATIVE="$LT_LOG/cafe-authored-manifest.corrupt.json"
    CAFE_NEGATIVE_LOG="$LT_LOG/cafe-authored-manifest.negative.log"
    "$PY" tools/cafe_authored_manifest.py \
      --repo . --ids "$CAFE_IDS" --check "$CAFE_MANIFEST" --output "$CAFE_COMPILED" \
      >"$CAFE_POSITIVE_LOG" 2>&1
    if [ "$?" -eq 0 ] && cmp -s "$CAFE_COMPILED" "$CAFE_MANIFEST"; then
      cat "$CAFE_POSITIVE_LOG"
      ok "cafe authored manifest regenerated byte-identically"
    else
      cat "$CAFE_POSITIVE_LOG"
...
for scene in m2_test reqlife_test player_agency_test player_touch_test player_replay_test p1t_social_plane_test p1a_affiliate_test p1b_cargo_manifest_test p1c_east_ocean_carrier_test p1d_scale_export_test p1g_manifest_transaction_test p1u_port_nav_test p1v_warehouse_observatory_test s4_replay_test space_test c1_locked_ortho_test save_load_test save_migration_test goals_test story_test event_prose_test; do
  SCENE_T0=$SECONDS
